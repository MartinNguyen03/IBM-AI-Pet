...
export default function WelcomePage({ navigation, route }) {
  ...
  const fetchContactsFromServer = async () => {
    try {
      const response = await fetch(`https://ibm-ai-pet.onrender.com/comms?userID=${userID}`);
  ...
  const handleGetLocation = async () => {
    ...
    try {
      await fetch('https://ibm-ai-pet.onrender.com/user/location', {
        method: 'POST',
        headers: {
    ...
    if (data.length > 0) {
      ...
      data.forEach(async (contact) => {
        if (contact.phoneNumbers && contact.phoneNumbers.length > 0) {
          try {
            await fetch('https://ibm-ai-pet.onrender.com/comms', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
  ...
  const handleGetEvents = async () => {
    ...
    if (calendars.length > 0) {
      ...
      events.forEach(async (event) => {
        try {
          await fetch('https://ibm-ai-pet.onrender.com/calendar', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
